...
def load_molecule_net(filepath, task):

    # load data
    _df = pd.read_csv(filepath)

    # get smiles and targets
    smiles = _df[MOLECULE_NET_TASKS[task]['smiles']].tolist()
    targets = _df[MOLECULE_NET_TASKS[task]['targets']].values

    # process targets
    if np.any(np.isnan(targets)):
        targets[np.isnan(targets)] = MOLECULE_NET_TASKS[task]['nan_replacement_value']
    targets = targets.astype(MOLECULE_NET_TASKS[task]['type'])

    # SPMM filter
    smiles = [_featurize(smile) for smile in smiles]
    targets = targets[[smile is not None for smile in smiles]]
    smiles = [smile for smile in smiles if smile is not None]

    # Check data length
    assert len(targets) == len(smiles)
        ...
        for root, _, files in os.walk(data_dir):
            for file in files:
                if file.endswith(".csv"):
                    ...
                    if '.csv' in full_path:
                        file_info.append((full_path, filename_without_ext))
    else:
