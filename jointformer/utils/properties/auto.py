    ...
    def from_target_label(cls, target: str, dtype: Optional[str] = None) -> BaseTarget:
        ...
        if target == 'qed':
            return getattr(importlib.import_module(
                "hyformer.utils.properties.smiles.qed"),
                "QED")(dtype=dtype)
        elif target == 'physchem':
            return getattr(importlib.import_module(
                "hyformer.utils.properties.smiles.physchem"),
                "PhysChem")(dtype=dtype)
        elif target == 'plogp':
            return getattr(importlib.import_module(
                "hyformer.utils.properties.smiles.plogp"),
                "PlogP")(dtype=dtype)
        elif target == 'guacamol_mpo':
            return getattr(importlib.import_module(
                "hyformer.utils.properties.smiles.guacamol_mpo"),
