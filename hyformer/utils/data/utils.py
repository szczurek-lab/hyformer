...
_PAD_TO_MULTIPLE_OF = 128
_LM_PREFIX_LENGTH = 2  # e.g., [TASK], [BOS]


def _pad_sequences(sequences: List[Union[torch.Tensor, List[int]]], max_length: int, pad_value: Any) -> torch.Tensor:
    if not isinstance(sequences[0], torch.Tensor):
        sequences = [torch.tensor(seq) for seq in sequences]
    batch_size = len(sequences)
    padded = torch.full((batch_size, max_length), pad_value, dtype=sequences[0].dtype, device=sequences[0].device)
...
def _get_special_tokens_mask(tokenizer: BaseTokenizer, input_ids: torch.Tensor) -> torch.Tensor:
...
def _mask_tokens(tokenizer: BaseTokenizer, inputs: torch.Tensor, attention_mask: torch.Tensor, prob: float) -> Tuple[torch.Tensor, torch.Tensor]:
    labels = inputs.clone()
    ...
    to_mask = torch.bernoulli(torch.full(labels.shape, 0.8, device=labels.device)).bool() & masked_indices
    inputs[to_mask] = tokenizer.mask_token_id
    to_random = torch.bernoulli(torch.full(labels.shape, 0.5, device=labels.device)).bool() & masked_indices & ~to_mask
    random_tokens = torch.randint(len(tokenizer), labels.shape, dtype=torch.long, device=labels.device)
...
def create_dataloader(
    ...
    **kwargs
    ...
    def collate_fn(batch: List[Dict[str, Any]]):
        ...
        padded_input_ids = _pad_sequences(input_ids, batch_max_len, pad_value=tokenizer.pad_token_id)
        ...
        if task == 'lm':
            ...
            model_input_ids, input_labels = _mask_tokens(tokenizer, padded_input_ids.clone(), padded_attn, mask_probability)
    ...
    resolved_workers = int(os.environ.get("SLURM_CPUS_PER_TASK", 0)) if num_workers is None else num_workers
    g = torch.Generator()
