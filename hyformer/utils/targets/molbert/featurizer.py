...
class MolFeaturizer(ABC):
    ...
    def __call__(self, molecules: Sequence[str]) -> Tuple[np.ndarray, np.ndarray]:
        return self.data_transform(molecules)

    def data_transform(self, molecules: Sequence[str]) -> Tuple[np.ndarray, np.ndarray]:
        """
...
class PhysChemFeaturizer(RDKitFeaturizer):
    ...
    def data_transform(self, molecules: Sequence[str]) -> Tuple[np.ndarray, np.ndarray]:
        features, valids = super().data_transform(molecules)
...
class PhyschemScaler:
    ...
    def data_transform(self, X):
        # data_transform each column with the corresponding descriptor
        transformed_list = [
    ...
    def transform_single(self, X):
        ...
        X = X[np.newaxis, :]
        transformed = self.data_transform(X)
...
class SmilesIndexFeaturizer(MolFeaturizer):
    ...
    def transform_single(self, molecule: str) -> Tuple[np.ndarray, bool]:
        """
        data_transform a single amino acid sequence
