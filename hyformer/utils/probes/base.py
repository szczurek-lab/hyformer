...
class ProbeBase(ABC):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        Initialize the ProbeBase class with hyperparameter grid and cross-validation settings.
        ...
        task_type : str
            Task type, either 'classification' or 'regression'. Defaults to 'classification'.
        verbose : int
    ...
    def _calculate_enrichment_factor(
        self, y_true: np.ndarray, y_scores: np.ndarray, fraction: float
    ) -> float:
        """
    ...
    def fit(
        ...
        if selection_metric is None:
            selection_metric = self._get_default_selection_metric()
    ...
    def _get_default_selection_metric(self) -> str:
        """
        Return the default selection metric for this probe type.
        """
        return "auprc" if self.task_type == "classification" else "rmse"

    def predict(self, X: np.ndarray) -> np.ndarray:
        """Predict scores.
        - Classification: positive-class probability
    ...
    def evaluate(
        ...
        if self.task_type == "regression":
            ...
            ef1 = (
                (y_test[sorted_indices[:top_1_percent]].sum() / top_1_percent)
                / (y_test.sum() / n)
                if y_test.sum() > 0
                else 0
            )
            ef5 = (
                (y_test[sorted_indices[:top_5_percent]].sum() / top_5_percent)
                / (y_test.sum() / n)
                if y_test.sum() > 0
                else 0
            )
            ...
            if verbose > 0:
                self._display_evaluation_results(
                    metrics, y_test, y_pred_proba, threshold
                )

                # Plot curves if requested
                if plot_curves:
                    self._plot_evaluation_curves(
                        y_test, y_pred_proba, figsize, self.task_type
                    )
        else:
    ...
    def _plot_evaluation_curves(
        ...
        if task_type == "regression":
            ...
            ax1.plot(
                fpr, tpr, "b-", linewidth=2, label=f"ROC Curve (AUC = {auroc:.3f})"
            )
            ...
            # Class Histogram
            pos_probs = y_pred[y_test == 1]
            neg_probs = y_pred[y_test == 0]

            ax3.hist(
                neg_probs,
                bins=30,
                alpha=0.7,
                label=f"Negative Class (n={len(neg_probs)})",
                color="red",
            )
            ax3.hist(
                pos_probs,
                bins=30,
                alpha=0.7,
                label=f"Positive Class (n={len(pos_probs)})",
                color="green",
            )
