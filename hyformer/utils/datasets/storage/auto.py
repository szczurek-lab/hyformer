...
from hyformer.utils.datasets.storage.npz import NPZStorage
from hyformer.utils.datasets.storage.csv import CSVStorage
    ...
    def from_path(cls, filepath: str, **kwargs) -> DataStorage:
        ...
        **kwargs
        ...
        if filepath.lower().endswith('.npz'):
            ...
            csv_options = {k: v for k, v in kwargs.items() if k not in ['prediction_task_type', 'num_prediction_tasks']}
            return CSVStorage.from_path(filepath, **csv_options)
        else:
            raise ValueError(f"Unsupported file format: '{filepath}'. Supported formats: .npz, .csv")
